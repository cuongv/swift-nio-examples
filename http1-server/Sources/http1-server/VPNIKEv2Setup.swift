...
class VPNServerSettings: NSObject {
  ...
  let vpnServerAddress = "127.0.0.1"
  let vpnRemoteIdentifier = "127.0.0.1" // In my case same like vpn server address
  let vpnLocalIdentifier = "phone@caf1e9*******.algo"
  let vpnServerCertificateIssuerCommonName = "127.0.0.1" // In my case same like vpn server address
}
